...
public class MakeCommand {
    ...
    private String createFile(String name, String type, ContentGenerator generator) {
        ...
        String simpleName = getSimpleName(name); // e.g., users
        String className = getString(type, simpleName);

        // Directory: properties -> src/main/java/org/booking/properties
        // If name is "users", package is "users".
        // distinct logic: users -> users
        // NOTE: users might want specific package logic. Let's try to infer or just use
        // lower case plural.
    ...
    private static String getString(String type, String simpleName) {
        ...
        if (!type.equals("Entity") && !type.equals("Enum")) {
            ...
            // present,
            // but user usually provides full name like UserServiceImpl.
            // Let's assume user provides "users" and we append specific suffix OR user
            // provides "UserServiceImpl".
            // Standard Laravel `make:controller users` creates `UserController`.
            // Let's check if name ends with Type.
            if (!className.endsWith(type)) {
                className += type; // users -> UserServiceImpl
            }
        }
        return className;
    }
    ...
    private String getSimpleName(String name) {
        return name; // Simplified for now, assuming just "users" passed
    }
    ...
    private String camelToSnake(String str) {
        // Simple lowercase for now as is common in Java packages (e.g. users -> user,
        // but plural -> users)
